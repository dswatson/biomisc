...
#' @param window Lower and upper bounds for the consensus index sub-interval over 
#'   which to calculate the PAC. Must be on (0, 1).
...
#' default settings of \code{window = c(0.1, 0.9)} are taken from the original PAC 
...
PAC <- function(cc, 
  ...
  maxK <- length(cc)
  if (!is.list(cc) || 
      any(map_lgl(2L:maxK, function(k) !'consensusMatrix' %in% names(cc[[k]])))) {
  ...
  cat(paste0('Optimal cluster number is k = ', PAC$k[which.min(PAC$PAC)], ', with a ',
